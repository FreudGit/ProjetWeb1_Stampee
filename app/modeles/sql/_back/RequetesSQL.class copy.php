...
class RequetesSQL extends RequetesPDO {
   ...
   * @param int $film_id, clé du film
   * @param boolean $admin jointure externe pour l'interface d'administration
   * @return array tableau des lignes produites par la select 
   */ 
  public function getActeursFilm($film_id,  $admin = false) {
    $this->sql = "
      SELECT acteur_id, acteur_nom, acteur_prenom, film_id, f_a_priorite 
      FROM acteur
      ". ($admin ? 'LEFT' : 'INNER') ." JOIN film_acteur ON f_a_acteur_id = acteur_id AND f_a_film_id = :film_id
      ". ($admin ? 'LEFT' : 'INNER') ." JOIN film ON  f_a_film_id = film_id
      ORDER BY film_id, f_a_priorite, acteur_nom, acteur_prenom";
   ...
   * @return array tableau des lignes produites par la select 
   */ 
  public function getPaysFilm($film_id, $admin = false) {
    $this->sql = "
      SELECT pays_id, pays_nom, film_id
      FROM pays
      ". ($admin ? 'LEFT' : 'INNER') ." JOIN film_pays ON f_p_pays_id = pays_id AND f_p_film_id = :film_id
      ". ($admin ? 'LEFT' : 'INNER') ." JOIN film ON  f_p_film_id = film_id
      ORDER BY film_id, pays_nom ASC";
    return $this->getLignes(['film_id' => $film_id]);
  }
   ...
   * @return array tableau des lignes produites par la select 
   */ 
  public function getRealisateursFilm($film_id, $admin = false) {
    $this->sql = "
      SELECT realisateur_id, realisateur_nom, realisateur_prenom, film_id
      FROM realisateur
      ". ($admin ? 'LEFT' : 'INNER') ." JOIN film_realisateur ON f_r_realisateur_id = realisateur_id AND f_r_film_id = :film_id
      ". ($admin ? 'LEFT' : 'INNER') ." JOIN film ON  f_r_film_id = film_id
      ORDER BY film_id, realisateur_nom, realisateur_prenom ASC";
    return $this->getLignes(['film_id' => $film_id]);
  }
  ...
  /* GESTION DES TABLES DE JONCTION POUR UN FILM 
     =========================================== */

  /**
   * Regénérer la liste des acteurs d'un film dans la table de jonction film_acteur
   * @param int    $film_id
   * @param string $acteursId, liste des acteur_id séparés par des virgules
   * @return array liste réactualisée
   */ 
  public function modifierActeursFilm($film_id, $acteursId) {
    try {
      $this->debuterTransaction();
      $this->sql = 'DELETE FROM film_acteur WHERE f_a_film_id = :film_id';
      $this->CUDLigne(['film_id' => $film_id]);   
      
      $tActeursId = $acteursId != '' ? explode(',', $acteursId) : [];
      $priorite = 0;
      foreach ($tActeursId as $acteur_id) {
        $this->sql = 'INSERT INTO film_acteur SET f_a_acteur_id = :acteur_id, f_a_film_id = :film_id, f_a_priorite  = ' . ++$priorite;
        $this->CUDLigne(['film_id' => $film_id, 'acteur_id' => $acteur_id]);
      }
      $this->validerTransaction();
      return $this->getActeursFilm($film_id);
    } catch(Exception $e) {
      $this->annulerTransaction();
      throw $e;
    }  
   ...
   * @param array $champs tableau des champs de la ligne à insérer 
   * @return int
   */ 
  public function ajouterPaysFilm($champs) {
    $this->sql = '
      INSERT INTO film_pays SET
      f_p_pays_id = :pays_id,
      f_p_film_id = :film_id';
      return $this->CUDLigne($champs); 
  }
   ...
   * @param array $champs tableau des champs de la ligne à supprimer 
   * @return int
   */ 
  public function supprimerPaysFilm($champs) {
    $this->sql = 'DELETE FROM film_pays WHERE f_p_pays_id = :pays_id AND f_p_film_id = :film_id';
      return $this->CUDLigne($champs); 
  }
   ...
   * @return int
   */ 
  public function ajouterRealisateurFilm($champs) {
    $this->sql = '
      INSERT INTO film_realisateur SET
      f_r_realisateur_id = :realisateur_id,
      f_r_film_id        = :film_id';
      return $this->CUDLigne($champs); 
  }
   ...
   * @return int
   */ 
  public function supprimerRealisateurFilm($champs) {
    $this->sql = 'DELETE FROM film_realisateur WHERE f_r_realisateur_id = :realisateur_id AND f_r_film_id = :film_id';
      return $this->CUDLigne($champs); 
  }
